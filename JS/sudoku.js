...
function level(event) {
    ...
    if(value === 1) {
        var soduko_level = 1 // The selected variable changing the level by "empty" cells, minimum one cell. 
    } else if (value === 2) {
        var soduko_level = 40
    } else if (value === 3) {
        var soduko_level = 60;
...
//פונקציה שמעתיקה את הלוח לתוך המשתנה
...
function isRowValid(matrix, rowIndex, columnIndex) { // Function that check if the row is valid
    ...
    for (var i = 0; i < matrix[0].length; i++) {
        ...
        // קולום אינדקס עובר מ 0 עד 8 בהתאם לתא הניבדק. התנאי לא מתקיים אם בודקים את התא בעצמו
